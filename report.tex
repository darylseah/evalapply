...
improvements and related developments~\cite{}. However we choose these systems because
...
Islands~\cite{hogg91islands} and Balloons~\cite{almeida97balloons}. The latter
works were early attempts at (essentially) solving the same problem but did not
provide sufficient expressivity and ease of use to be practical. In particular,
objects could not play multiple roles in both Islands and Balloons, and
Balloons required the programmer to deal with complex abstract
interpretation~\cite{noble98alias}.



% islands of objects - guarantee that an object is not aliased.
%
%
% balloons - controlling shared state.
%
%
% flexiable alias protection - limiting where an alias could propgate and limiting changes observable through an alias.
%
% ownership is a formalism of flexible alias protection.
%
% objects can play different roles in flexible alias protection.
% for example, containers can hold refs to value objects that it doesn't own
%   . In balloon/islands, everything refererenced by ta container must be part of the rep??? (confirm this)
%
%
%
%
% balloon types = complex abstract interpretation, not intuitive for programmers
