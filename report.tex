...
% These are just to get rid of hyperref warnings
...
specifiers. Using Boyapati's SafeJava\cite{boyapati04safejava} syntax, the Car 
example can be expressed as per \cref{code:ownership_types_car_engine_1} 
...
enforced statically. All types are now parameterised by an owner type, 
preventing objects with different owners from being assigned to each other. 
Furthermore, it is not possible to express the owner type of an object 
belonging to another object, preventing the encapsulation violation 
