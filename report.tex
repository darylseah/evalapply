...
\begin{lstlisting}[
	caption={Modular reasoning: Car Engine},
...
class Main {
	static void main() {
		Car car = new Car();
		car.start();
		car.engine.start();/*@\label{modular_reasoning_car_engine_1_engine_start}@*/
	}
}
\end{lstlisting}
...
only be started if a driver is present. However, on line 
\ref{modular_reasoning_car_engine_1_engine_start} client code that created the 
car was able to start the engine bypassing that check. This should not be 
allowed because it has broken the encapsulation discipline.
