...
were ignored. A summary of our findings is listed in \cref{tab:summary}.
...
\begin{enumerate}
	...
	\item \textbf{Mutable views} (\texttt{mutable-view}): This example involves
		a linked list and an iterator which can \lstinline|remove()| the
		element that the iterator points to. This mirrors Java's implementation
		of iterators.
...
following the definition given in \cref{subsec:definitions} is generally too
strict to be able to capture the notion that an object is able to explore the
representation invariant of another, even in a controlled way. The
\hyperref[item:mutable-views-as-inner-classes]{Mutable views as Inner classes}
example was therefore specifically constructed to overcome this problem,
capitalising on special privileged access~\cite{boyapati04safejava} or multiple
ownership \cite{cameron07mojo}.
