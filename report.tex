...
% using deprecated package because subcaption does not work with ACM template
...
previously described: either Projects or Workers may own tasks, so only
...
\textbf{owners-as-modifiers} discipline still applies in principle, allowing any
one of an object's multiple owners to modify it.\footnote{In practice, however,
the implementation of multiple ownership discussed in the paper, MOJO, does
not enforce owners-as-modifiers; it has been left to future work.} Multiple
ownership is reified with several syntactic additions:
...
\begin{itemize}
	...
	\item \textbf{Wildcards} (or \textit{existential owners}) allow programmers
		...
		notable difference from Java's generic type parameters, which are
		\textit{invariant}.  \lstinline|Task<a & b>| is considered a subtype of
		both \lstinline|Task<a & ?>| and \lstinline|Task<b & ?>|, allowing it to
		be used wherever the latter two are expected.}
...
In this section we evaluate each of the systems that we were introduced in
\cref{sec:systems}. We first examine the expressivity of each system, because a
system that is too restrictive would prevent widespread adoption. We then look
...
\begin{figure*}[t]
	\centering
	\subfigure[Inside Representation]{
		\includegraphics[width=0.97\columnwidth]{iterator-fail-inside.eps}
	}
	\hspace{\columnsep}
	\subfigure[Outside Representation]{
		\includegraphics[width=0.97\columnwidth]{iterator-fail-outside.eps}
	}
...
\Cref{fig:iterator-fail} highlights the limitations well: following the
...
Typically, in an object-oriented program, a binary operator is implemented as a
method of the values' class and invoked with one value as the receiver. The
actual operation is performed with respect to the recipient. It follows that the
recipient must have access to the internals of the other in order to carry
out the operation. If there are any changes in the state of the objects, they
will typically be to the recipient, hence read-only access to the other is
...
programmer from implementing such an operation as read access is sufficient.

Universe Types allows objects with a \lstinline|peer| \cite{dietl11gut}
relationship to explore each other's representations. Therefore, binary
operators can be implemented even if \lstinline|peer| access is not granted.
...
Read access is granted even with the \textbf{owners-as-modifiers} discipline, as
...
\begin{table*}[t]
\caption{Summary of Expressivity and (Analytical) Benefits of Ownership Type Systems}
\label{tab:summary}
\centering
...
Programming languages integrated with an ownership type system can provide a
number of benefits for program analysis. Specifically, in terms of effects,
alias control and local reasoning.

\subsubsection{Effect Specifications}
\label{subsubsec:benefits_effect_specifications}

SafeJava, Universe Types, and Multiple Ownership include effect specifications
in some form. Effect specifications are useful because they allow analysis
tools to make stronger assumptions about programs, increasing the accuracy of
analyses.

%TODO check again

Universe Types gives the simplest facility for effect specification: the
\lstinline|pure| and \lstinline|impure| method annotations to indicate the
presence of side effects. This is used to constrain methods that may be invoked
on objects with the given ownership properties: impure methods are not
permitted in contexts where only read access is available.

SafeJava provides finer-grained control, using \lstinline|reads| and
\lstinline|writes| clauses listing individual members of a class that a method
operates on. This is used to the same effect as in Universe Types: restricting
the possible method calls that can be made on objects based on their ownership
properties.

Multiple Ownership uses \lstinline|intersects| and \lstinline|disjoint|
clauses, which express the relationships between ownership contexts, to
give a conservative, sound, \textit{must} analysis of the areas of the heap
written to or read by an expression. This information is also used to verify
that methods do not violate their encapsulation discipline. Information on the
effects of methods is assumed to be present, therefore in practice it requires
some means of distinguishing pure methods from impure ones.
