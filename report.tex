...
program verification applications --- it is sufficient to simply restrict the
references that may modify an object to those held by its
owners~\cite{dietl11gut}. In other words, to enforce the
\textbf{owners-as-modifiers} discipline instead of owners-as-dominators,
thereby giving programmers greater expressibility. For example, read-only
iterators and binary operations are not an issue in Universe
...
object assignment (e.g.\ \lstinline|a = b;|) is legal if the ownership type of
\lstinline|a| is expressible and is identical to or a generalization of the
type of \lstinline|b|. Note that nothing can be assigned to a \lstinline|lost|
reference since the ownership information required for verification is missing,
but we can assign anything to \lstinline|any| (including \lstinline|lost|)
since the ownership information will be ultimately discarded.
...
Boyapati's~\cite{boyapati04safejava} work. However, when Clarke and Boyapati
...
generic type syntax of Java.

% example of generic syntax

% quite popular. relatively easy to understand. not that limited in expressibility. many implementations 
