...
reason about its behaviour and effects. Here, we present an example, originally 
presented by \cite{clarke98ownership}:
...
appropriate scope. The following example, modified from 
Listing~\ref{code:modular_reasoning_car_engine_1}, demonstrates that enforcing 
...
In this example we annotate each method and member variable with an appropriate 
access specifier. We also define an additional \emph{accessor} method, for the 
situation where a property of the car's engine must be accessed outside of the 
car (an instrument attached to the car's electronics to measure performance, 
for example). However, as seen on 
