...
\begin{itemize}
    ...
    \item The owner-as-dominator discipline is lost. This admits more programs
        which violate encapsulation as access paths are no longer restricted to
        one dominator.

    \item Ownership constraints reflect the implementation of ownership in the
        form of sets. Their semantics are difficult to reason about without
        understanding of the view of ownership contexts as sets, and may not be
        as easy to use as other type systems.

    \item As multiple ownership is work in a new direction, it has also not been
        fully generalised. Notably, work on integrating it with generics and
        race detection is ongoing.
